"""Looking for a newer Brain Waves, and putting it in place.

Village leaders run a single downloaded file, so an update is one file replacing another.
The running program renames itself out of the way and moves the download in, which Windows
allows for a file that is open and which the other two allow outright.
...
ICON = "brainwaves.png"  # what the Linux package calls the icon the desktop entry uses
EXECUTABLE = "brainwaves"  # what every build's executable is called, plus its platform
...
def download(release: Release) -> Path:
    ...
    try:
        ...
        response.raise_for_status()
        handle, name = tempfile.mkstemp(prefix="brainwaves-", suffix=Path(release.url).suffix)
...
def install(downloaded: Path) -> Path:
    ...
    if not getattr(sys, "frozen", False):
        raise UpdateError(
            "This copy of Brain Waves runs from source, so update it with git instead."
        )
...
def _unpack(archive: Path) -> Path:
    ...
    folder = Path(tempfile.mkdtemp(prefix="brainwaves-update-"))
    try:
