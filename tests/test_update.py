...
from brainwaves.update import Release, UpdateError, install, is_newer, latest_release
...
def _release(monkeypatch, payload):
    monkeypatch.setattr("brainwaves.update.requests.get", lambda *a, **k: Response(payload))
...
def test_a_newer_release_for_this_platform_is_offered(monkeypatch):
    monkeypatch.setattr("brainwaves.update.sys.platform", "linux")
    release = _release(
        monkeypatch,
        {
            "tag_name": "v9.9.9",
            "body": "Faster dragging",
            "assets": [
                {"name": "brainwaves-windows.exe", "browser_download_url": "https://w"},
                {"name": "brainwaves-linux", "browser_download_url": "https://l"},
            ],
        },
    )
    assert release == Release("9.9.9", "https://l", "Faster dragging")


def test_the_current_version_is_not_offered(monkeypatch):
    payload = {
        "tag_name": f"v{__version__}",
        "assets": [{"name": "brainwaves-linux", "browser_download_url": "https://l"}],
    }
...
def test_a_release_without_a_file_for_this_platform_is_not_offered(monkeypatch):
    monkeypatch.setattr("brainwaves.update.sys.platform", "linux")
    payload = {"tag_name": "v9.9.9", "assets": [{"name": "brainwaves-windows.exe"}]}
    assert _release(monkeypatch, payload) is None


def test_a_failed_check_says_so(monkeypatch):
    def explode(*a, **k):
        raise OSError("no network")

    monkeypatch.setattr("brainwaves.update.requests.get", explode)
...
def linux_package(tmp_path, executable="brainwaves-linux", icon=True):
    ...
    if icon:
        (package / "brainwaves.png").write_bytes(b"new icon")
    archive = tmp_path / "brainwaves-linux.tar.gz"
    with tarfile.open(archive, "w:gz") as tar:
        tar.add(package, arcname="brainwaves-linux")
    return archive
...
def installed(tmp_path, monkeypatch, icon=True):
    ...
    home = tmp_path / "opt"
    home.mkdir()
    running = home / "brainwaves-linux"
    running.write_text("old")
    if icon:
        (home / "brainwaves.png").write_bytes(b"old icon")
    monkeypatch.setattr("brainwaves.update.sys.frozen", True, raising=False)
    monkeypatch.setattr("brainwaves.update.sys.executable", str(running))
    return running
...
def test_a_linux_release_is_unpacked_and_the_program_inside_it_installed(tmp_path, monkeypatch):
    ...
    assert (running.with_name("brainwaves-linux.old")).read_text() == "old"
    assert (running.parent / "brainwaves.png").read_bytes() == b"new icon"
...
def test_an_icon_is_only_refreshed_where_there_is_one(tmp_path, monkeypatch):
    ...
    assert not (running.parent / "brainwaves.png").exists()
...
def test_a_bare_executable_is_still_installed_as_it_was(tmp_path, monkeypatch):
    ...
    running = installed(tmp_path, monkeypatch, icon=False)
    downloaded = tmp_path / "brainwaves-macos"
...
def test_an_archive_with_no_program_in_it_says_so(tmp_path, monkeypatch):
    ...
    with pytest.raises(UpdateError, match="holds no brainwaves-linux"):
        install(archive)
